...
public partial class DFDEditor
{
    ...
    private bool HandleMultiConnectClick(int nodeId, string side, int position)
    {
        ...
        if (multiConnectSourceNode == null)
        {
            ...
            multiConnectSourceNode = nodeId;
            multiConnectSourcePoint = connection;
            
            // Get coordinates for visual feedback
            var node = nodes.FirstOrDefault(n => n.Id == nodeId);
            if (node != null)
            {
                multiConnectSourceCoords = GeometryService.GetConnectionPointCoordinates(node, side, position);
            }
        ...
        else
        {
            ...
            if (nodeId != multiConnectSourceNode) // Don't connect to self
            {
    ...
    {
        if (multiConnectSourceNode == null || multiConnectSourcePoint == null) return;
        ...
        {
            Id = nextEdgeId++,
            From = multiConnectSourceNode.Value,
            To = destNodeId,
            FromConnection = multiConnectSourcePoint,
